...
lastmod: 2023-08-14T14:31:12+08:00
draft: false
weight: 1002
---
...
3.  考虑到左右溢出和窗口宽度, 图片水平位移范围: <br/>
    ```javascript
    let frontEnd = 2000; // > window.innerWidth
    let backEnd = -460;
    ```
...
let cur2 = 0;
let scrollTimer = window.setInterval(function () {
  cur1 = (cur1 + step) % (frontEnd - backEnd);
  cur2 = (cur2 - step) % (frontEnd - backEnd);
